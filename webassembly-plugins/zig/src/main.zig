...
export fn hook_client_query_received() i32 {
    ...
    const log_msg = std.fmt.allocPrint(
        allocator,
        "Received query from {s} for {s}",
        .{input.client_ip, input.query_name}
    ) catch unreachable;
    ...
    for (blocked_domains) |domain| {
        if (isDomainOrSubdomain(input.query_name, domain)) {
            const block_msg = std.fmt.allocPrint(
                allocator,
                "Blocking query for {s} from {s}",
                .{input.query_name, input.client_ip}
            ) catch unreachable;
